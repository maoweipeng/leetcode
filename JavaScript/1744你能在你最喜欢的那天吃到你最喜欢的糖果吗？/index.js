 ...
 * 其中 candiesCount[i] 表示你拥有的第 i 类糖果的
 * 数目。同时给你一个二维数组 queries ，其中 
 * queries[i] = [favoriteTypei, favoriteDayi, dailyCapi] 。
 * 你按照如下规则进行一场游戏：你从第 0 天开始吃糖果。
 * 你在吃完 所有第 i - 1 类糖果之前，不能吃任何一颗
 * 第 i 类糖果。在吃完所有糖果之前，你必须每天至少吃
 * 一颗糖果。请你构建一个布尔型数组 answer ，满足
 * answer.length == queries.length 。
 * answer[i] 为 true 的条件是：
 * 在每天吃不超过 dailyCapi 颗糖果的前提下，你可以在
 * 第 favoriteDayi 天吃到第 favoriteTypei 类糖果；
 * 否则 answer[i] 为 false 。注意，只要满足上面 3 条
 * 规则中的第二条规则，你就可以在同一天吃不同类型的糖果。
 * 请你返回得到的数组 answer 。
 * 
 * 示例 1：
...
var canEat = function (candiesCount, queries) {
  const counts = new Array(candiesCount.length + 1)
  counts[0] = 0
  for (let i = 0; i < candiesCount.length; i++) {
    counts[i + 1] = counts[i] + candiesCount[i]
  }
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    result.push(counts[favoriteTypei + 1] >= favoriteDayi + 1 && counts[favoriteTypei] < dailyCapi * (favoriteDayi + 1))
  }
